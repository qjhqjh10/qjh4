...
| `Unity/MyGame/Assets/CardPresentation/Scenes/` | 15 MB | 卡牌基座演示场景 —— 占位卡面是运行时生成的贴图，存场景时被烘进去了 | `CardBaseDemo.Run` |
...
2. **`Unity/资料/对战排版_原版数值与改造方案.md`** —— **🆕 下一轮主攻**：原版手牌布局器字段+曲线、棋盘坐标（含镜像）、卡面数值实测位置、逐项改造步骤
3. **`Unity/资料/规则引擎_进度与交接.md`** —— **玩法线**：游戏已能玩、怎么跑自检、自设计的两套卡、下一步
4. **`Unity/资料/特效还原_进度与交接.md`** —— **特效线**：已验证的事实、产物、已知 bug、**下一步（P1-a0 + 🆕 P1-a1）**
5. **`Unity/资料/卡牌基座_进度与交接.md`** —— 表现层：布局/卡/拖拽/落位，设计决策与踩过的坑
6. **`Unity/资料/规则引擎_设计.md`** —— 规则引擎的边界、数据结构、逐条规则出处
7. **`Unity/资料/特效还原台账.tsv`** —— 957 行逐效果状态（判定 / 亮度比 / `|ln|` / 置信度 / 技术构成）
8. `Unity/资料/自研游戏_特效与卡牌基座_设计.md` —— 设计草案：模块划分、接口草案、落地顺序
9. `Unity/_资源评估_场景特效动画.md` —— 场景/特效/动画三类的可用性判定与逐项工时
...
- ✅ 对战自检 **22/22 全过**（含真实的拖拽路径）+ 规则引擎自检 **135/135 全过**
- ⬜ **下一轮主攻「对战排版」** —— 现在版面还是自己拍的。原版的手牌布局器参数、
  棋盘坐标（含**敌方镜像**）、卡面数值位置**都已找到**，见
...
- ✅ **特效「能用」了** —— `WarpforgeEffectPlayer`（播放/生命周期）+ `WarpforgeEffectLibrary`（958 条）
  + 白板自检（**闭环 21/24、渲染与台账一致 24/24、5 轮无泄漏**）
- ✅ 修掉 C 组主因（精灵没导出 → 遮罩粒子整块渲成空），并**修掉了尺子本身的不确定性**
- ⬜ **下一步主攻 P1-a0：自建抓屏扭曲和原版不等价**（高置信里 **102 个**带它）
...
- ⬜ 战术卡效果（478 张卡）/ 关键词补全（还有 47 个）/ 对手强一点 / 中文卡面 / 换真美术

**四条线都欠**：构建后 player 验证（到现在只验过编辑器）
