...
## 六、🆕 交叉校验：**独立的第二法 19/19 完全一致**

同一分片代理之后又用**另一套方法**把这 19 张重判了一遍，**逐张结果完全相同**：

| | 法 A（本文件用的：外圈带限制、无搜索） | 法 B（**全带 + 局部 (S, ox, oy) 搜索**） |
|---|---|---|
| 限制区 | 只在「框 `alpha≥250` 且卡外圈带」内算 MAD | **全带**都算 |
| 对齐 | 按基座矩形缩放着色后比对 | **每档各自做最优缩放/偏移搜索** |

两法**19/19 一致**，而且：
- **两法都不曾把任何一张判到 tier3 / tier4**；
- **两法都不曾把 tier1 组与 tier2 组混判**
⇒ 「tier1 vs tier2」这个二分**是真的，不是拟合噪声**。

**把握度修正**（因双法一致而上调，但两法 margin 都小的仍保守）：

| 卡 | 法A margin | 法B margin | 现把握 |
|---|---|---|---|
| `Vial Tanks` · `Pilfered Supplies` · `Rusted Vent` | 4–7 | **1.9–2.6** | 中 → **中**（上调后维持保守） |
| `Light Cover` | 3.3 | **1.9** | **低**（两法绝对拟合最差；画面目视确为最简素框 ⇒ 更偏 tier1） |
| 其余 12 张 | 高 | 高 | **高**（含 4 张目视确认） |

> ⚠️ 结论要点不变：**没有一张 `special` 卡用 tier4**；**框档不跟稀有度**
> （`CardTier{Tier1..Tier4}` 与 `CardRarity{…,Special=5}` 是独立枚举，`GetClanFrame` 收的是 `CardTier`）。
