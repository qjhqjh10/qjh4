...
## 四之二、🔴 表现层：**选牌 / 选效果 / 换牌 三者的面板关系**（2026-09-13 派子代理查实）
...
✅ **⇒ 这个待办 2026-09-14 晚已做完（本段原来是「已登记待办」）：**

> **结果**：两块面板的**展开实况都拍到了** —— 换牌 `Mulligan`（原版自己弹的，没催）·
> 选牌/选效果 `ChooseCardMenu`（探针 `SetActive` 摆出来的，**有催**）。
> 截图 + 完整运行时节点树（sprite 名/文字/颜色/字号/activeSelf）+ 原版 `GetPosition` 布局数学，
> **全在 `资料/原版参照图/Unity参照管线_0825/data/panel_0914b/报告.md`**（**唯一**出处，别在别处抄第二份）。
> ⚠️ 仍然**没拿到卡**：`GetEnvCardList()` 返回的元素全是 null、卡 prefab 走 Addressables（离线不可用）、
> 本地 bundle 里 `RawCardScript` = 0 ⇒ **「每张卡长什么样」依旧只有静态推导值**。
> ⚠️ 同时**推翻了旧结论**：UI 拍不到的真因不是「URP 下 RT 拍不到」，是**离线态 UI 相机一张都不渲**；
> 拍法（重指 canvas 到会渲的相机 + **等 3 帧** + 关掉 `LoadingMenu`/`Tutorial`）见报告 ①。
> 表现层动手（抽 `MulliganPanel` 的公共部分）**之前**先做这一步 —— 现在可以动了。
