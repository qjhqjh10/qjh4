...
🔴 **⇒ 这是一个已登记的待办（用户 2026-09-14 点名「请你记录，以后要做」）：**

> **跑原版游戏，把这两个面板展开的样子拍下来**（`ChooseCardMenu` 与 `Mulligan` 各一次）。
> 跑法见 `项目任务.md` §八「进原版游戏」（`d:/2/unity_run_ref/` + `sjs_dump_cfg.txt`，
> 要先设 `DOTNET_ROOT`、副本路径必须纯 ASCII）。
> ⚠️ 难点：面板要**有内容才展开** —— 而手牌注入在原版**做不到**（关服、卡数据在远程 CCD，见铁律 4 那张表）；
> 换牌阶段（`Mulligan`）**开局就会自己弹**，那一次是有实况可拍的；选牌/选效果要看**能不能触发到那个阶段**。
> **拍不到就如实记「没拍到」**，别拿静态字段冒充运行时结论。
> 表现层动手（抽 `MulliganPanel` 的公共部分）**之前**先做这一步。
