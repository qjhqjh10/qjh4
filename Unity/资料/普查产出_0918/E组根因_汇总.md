...
## 〇之〇、✅ **重扫已完成（2026-09-18 20:39）—— 本文所有计数都已过期，看这一节**

命令：`WFSWEEP_SIDE=orig` 一趟 → 不设再一趟 → `analyze_sweep.py`（**exit 全 0**）；
跑前已备份旧基线到 `_tmp_view/sweep_backup_0918/`（含 md5）。

| 分组 | 旧台账（09-15） | **新台账（当前构建）** |
|---|---|---|
| **E（偏亮/偏暗）** | **242** | **151**（偏亮 84 / 偏暗 67） |
| **Z（对得上）** | **491** | **580** |
| C（只有原版有内容=导出整个丢了） | 1 | 3 |
| D（只有导出有内容） | 33 | 29 |
| W（两边全程空） | 190 | 194 |

**指标**：全样本 `|ln|` **中位 0.092**（旧 0.170）· E 组中位 **0.560**（旧 0.698）· **79% 落在 0.7–1.4 窗口**。
尺子自检（Z 随带宽边界变）：`[0.80,1.25]`→477 · `[0.75,1.33]`→528 · `[0.70,1.40]`→580 · `[0.60,1.50]`→621
（⚠️ 边界挪 0.05 就摆动几十个 ⇒ 比它小的变化说明不了任何事）。

⇒ **「先重扫」这一步是对的**：09-17 的重导 + 09-18 的白名单确实让资产变好了一大截（E −91 / Z +89）。

### 🔴 而且它对本文的**头号结论**给了反证 —— 必须按新账重核

本文 §一 说的「锅 H₁（我们自建 shader 多一个 `_EmissionColor`）」是**按旧账的 242 条**量出来的。
拿**新账**做统计检验（`工具/_shader_impact_by_group.py`）**不支持它是主因**：

| | E 率 |
|---|---|
| 用了 `Everguild/FX/Extra Color`（555 个） | **21.4%** |
| 没用（176 个） | **18.2%** |
| **全池基线** | **15.8%**（151/957） |

E 组内部对照也几乎一样：用了的 n=119 中位 **0.543** / 没用的 n=32 中位 **0.619**。
⇒ 那个机制**可能仍然真实**（我们的 shader 确实多一个属性、确实会多加一份），
但**它不是把 151 条救出 E 组的钥匙**。**别照本文 §一 直接动手。**

### 🔴 按新账重新找线索：E 率最高的三个原版 shader

（只列 n≥15 的；全池基线 15.8%）

| 原版 shader | E 率 | 我们的处置 |
|---|---|---|
| **`Sprites/Default`** | **29.9%**（35/117） | 导出表里**映射到它自己**（Built-in 老 shader 原样留用） |
| **`Sprites/Mask`** | **28.7%**（35/122） | 同上 |
| **`Everguild/FX/Particle Dissolve Mask`** | **27.6%**（8/29） | `Replacements` 里映射到**自建 `WarpforgeVFX/Particles/Extra Color`**（`WarpforgeShaderMap.cs:91`） |
| `Everguild/FX/Particle Shine Custom Vertex Streams` | 19.2%（5/26） | — |
| `Mobile/Particles/Alpha Blended` | 17.6%（28/159） | — |
| `Everguild/Matcap/Matcap Full Options` | 16.7%（20/120） | — |
| `Everguild/FX/Extra Color` | **16.1%**（119/740） | 自建替代（= §一 那条） |
| `Universal Render Pipeline/Particles/Unlit` | 15.8%（149/946） | — |

⇒ **下一个该查的是前三个**（E 率接近全池两倍），不是 §一 那个。

### 🔴 但**做完第二道混淆控制之后，三个候选只剩一个**

「E 率」是比例，但如果那些效果本来就**更复杂**（用到的 shader 种数更多），而 E 率本来就随复杂度升，
那高 E 率就说明不了什么。⇒ **按「用到的原版 shader 种数」分桶、在同桶内比**
（这一步已固化进 `工具/_shader_impact_by_group.py`，它会自动打两道控制）：

| 原版 shader | 同桶内对照 | 判定 |
|---|---|---|
| **`Sprites/Default`** | 4 种 **35.7% vs 20.4%** · 5 种 **30.0% vs 8.8%** · 6 种 **41.7% vs 16.1%** | ✅ **一致地高 ⇒ 真信号** |
| **`Sprites/Mask`** | 与上**命中同一批 35 条**（Mask 是配套的遮罩 pass） | ✅ **同一个信号，不是两个** |
| `Everguild/FX/Particle Dissolve Mask` | 3 种 44% vs 16% · 4 种 33% vs 15% · **5 种 0% vs 10% · 6 种 0%** | ⚠️ **方向乱、小样本 ⇒ 存疑，别追** |
| `Particle Shine CVS` · `Mobile/Alpha Blended` | 只在个别桶高一点 | ⚠️ 弱 |
| `Matcap Full Options` | 各桶都在基线附近 | ❌ 无信号 |
| **对照：E 率 vs shader 种数本身** | 14.3 / 19.0 / 17.6 / 16.0 / 9.7 / 17.8 / 21.6% | **没有单调趋势 ⇒ 复杂度不是混淆项** |

❗ **同时排除掉一条**：本来怀疑 `Particle Dissolve Mask` 是「白名单没生效」——**不是**。
重扫的 `exp` 趟日志逐条印着
`shader 'Everguild/FX/Particle Dissolve Mask' → 原版bundle（白名单）` ⇒ **白名单确实解析到了原件**，
那 8 条**不是 shader 的问题**，别往那儿修。

⇒ **唯一在追的是 `Sprites/Default` + `Sprites/Mask` 那一族 35 条**，机制调查见
`资料/普查产出_0918/E组_SpritesMask族.md`。

### 🔴 但它也被否掉了（2026-09-18，子代理拿 3 个干净样本逐条核）—— **本文这两轮的「候选」全部落空**

**`Sprites/Default` 不是机制，只是个标记**：这个 prefab 带一个 `Sprite Mask` 节点，
那条 `Sprites/Default` 材质属于遮罩节点上 **`m_Enabled: 0` 的 `SpriteRenderer`**（根本不画），
`Sprites/Mask` 属于同一 GameObject 上的 `SpriteMask` ⇒ **两者永远同进同出**（所以「两个候选」其实是一个）。

| 检验 | 结果 |
|---|---|
| 结构差异 | **0**：三样本组件直方图完全一致；全池 **941/957 个 prefab 整树逐渲染器相同**（另 16 个是脚本的重名/路径伪差） |
| 遮罩是不是活的 | **死的**：那 3 条所有渲染器 `m_MaskInteraction = 0`（**原版原始 typtree 核过**）⇒ `SpriteMask.enabled=false` 一个像素都不改 |
| 方向 | **不偏**：本族 19 亮 / 16 暗 = 54% / 46%，与全池 E 组（84/67）一致 ⇒ 「我们多画一层」会**单向**偏，实测不偏 |
| 形状 | **强度型**不是面积型：62.9% 越阈是强度、25.7% 是面积；`FrenziedEffect` 0.75s 处 **lit 1747 vs 1759（覆盖几乎相同）而 sum 1377→3564（2.6×）** |
| 遮罩时间点 | ⚠️ **更正一条前提**：台账那 8 个采样点**不走 `EffectIso`**（`EffectSweepBatch.RenderAt:347` 从不碰 `SpriteMask`）⇒ 遮罩**两侧都开着**，仍无口径差 |

⇒ **那 3 倍是「家族混淆」**（card-draw / trait-icon 图标类），与 `SpriteMask` 无关。
🔴 **教训**：我做了两道混淆控制（按 `分组`、按 **shader 种数**），**两道都过了，仍被判据** ——
因为真正的混淆是**第三个维度：效果家族**。**「控制了两个混淆项」不等于「控制住了」。**

### 结论：重扫后这一轮找的**三个候选全部落空**

`Particle Dissolve Mask`（白名单其实生效）· `Sprites/Default`/`Sprites/Mask`（家族混淆）· `Matcap Full Options`（无信号）。
⇒ **现在没有一条经得起检验的「当前构建 E 组 151 条」的成因**。
下一步要么按**家族 + 发射器数 + 可见主导槽 shader** 三维重新分组（子代理给的判据），
要么直接做**干预实验**：改 `_EmissionColor`（默认值改黑 / 删加法项）**再扫一次** ——
用 A/B 直接量，而不是继续在统计上推断。

## 〇、⚠️ 本节以下写于重扫**之前**，计数按旧台账（242 条）；**逐效果结论要按新账重核**
