...
## 四、✅ 2026-09-18：两个大头的「量过再动」**已量**，结论是**先别动它们**

**量法（零成本、可复跑）**：`工具/_shader_impact_by_group.py "Everguild/FX/Extra Color"` ——
拿**当前台账**按「这个效果用没用它」分组比，并且**按台账的 `分组` 控制混淆**。
输入是两份现成产物（`效果_shader对账.tsv` × `特效还原台账.tsv`），**不跑 Unity**。

| 分组 | 用了 `Extra Color` | 没用 |
|---|---|---|
| 全样本 | n=555 中位 **0.231** | n=177 中位 **0.050** |
| **E**（偏亮/偏暗，台账判定 169+73） | n=196 中位 **0.701** | n=45 中位 **0.624** |
| **Z**（对得上，491） | n=359 中位 0.072 | n=132 中位 **0.013** |

🔴 **全样本那个「差 4.6 倍」是分组构成混淆出来的假信号** —— **一控制 `分组` 就塌了**：
E 组里 0.701 vs 0.624（**只差 12%，而两者都远在 0.336 参考线之上**）；Z 组里 0.072 vs 0.013（都远在参考线之下）。
**成因**：用了它的效果里，落在「本来就差的 E 组」的比例高得多。

⇒ **这两个 shader 不是瓶颈，改它们修不好 E 组。** 把 `UseOriginal` 白名单扩到这两个（原计划）**优先级下调** ——
真要做也得先有「改完 E 组变好」的证据，而按上表看**它不会有**。

🔴 **真正的量在 E 组那 241 个**（偏亮 169 / 偏暗 73，中位 `|ln|` **0.698**），
而且 **209/241 是普通「精灵图 N 发射器」**（不含 Matcap/拖尾/抓屏扭曲）⇒ **锅不在古怪 shader 上**。
⚠️ `资料/可并行任务清单.md` ① 记的锅 A/F/G（trailMaterial 35 · prefab 发射器不画 16 · Matcap 属性表 13）
**是按「旧台账」分的组**，要按当前台账**重核**。
