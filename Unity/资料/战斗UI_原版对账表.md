...
| **🆕 名牌（督军信息条）的位置** | `子代理读报_back左区_0827.md:46`：`NameBackground` 以 `PlayerInfo` 为**中心**摆（`pos(0,0)`、anchor(0.5,0.5)），渲染宽 382.4 → **左缘 x = −11.5**；`PlayerInfo` 中心 y 从下 **65.5**（绝对 y[977,1052] 从上） | 我们 `BattleDriver` 把名牌按 `PlayerInfo` 的**左缘**（x=32）+ 中心 y=102.6 摆（出处是 `arena_hud_layout.py` 那句「PlayerInfo (32,977) 260×75」—— 把左缘和中心混在一句话里了） | ⚠️ **2026-09-13 发现，两边差 (右 44 px, 高 37 px)**，**没动**：本批四项按**绝对坐标**摆（里程碑），所以将来对齐名牌时它们不用跟着改。要修就改 `_myPlate`/`_myText` 那两行。⚠️ 注意 `(157,108)` / `(32,977)` 那两个数来自 `FrontCanvas/Alliance Panel` 底下的**另一份** `EnemyInfo`/`PlayerInfo`（inactive），**不是** `LeftArea` 下 HUD 那一份 |
