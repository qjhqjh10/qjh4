...
| **边角按钮群** | `SettingsBtn` 63.9²、`ChatButton` 64.4×61.8、`ShowCemeteryBtn` 64.5×64.2、`CenterCameraButton` 64.4×61.8、`OffensiveButton` 109×106.9 | ① `.../LeftArea`、`.../RightArea` | ⚠️ 三件做完：`SettingsBtn` ✅ 2026-09-13（开设置面板）、`ShowCemeteryBtn` ✅ 2026-09-13（开墓地日志）。**还差三个**：ChatButton / CenterCameraButton / OffensiveButton |
...
| | | ⚠️ **2026-09-13 更正**：最后半句是错的 —— `40k_bt_underbutton` **在**，但 **`UI_Button_Mulligan` 三态当时并不在工程里**（缓存里好好的、是没同步）。已补进 `工具/sync_battle_ui_art.py` 并同步。**换牌也已经做完了**，逐项与出处见 `资料/规则引擎_进度与交接.md` 第二十六轮 | 实测 |
