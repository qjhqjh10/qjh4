...
| **选卡菜单** | `ChooseCardMenu/ButtonsGroup`：Continue 548×75.9 + BG 577.5×63.8 + 圆钮 80.5×79.6 | ① `.../FrontCanvas` | ✅ **2026-09-14 已做**（选牌 / 三选一 / 选效果**三族共用一个面板** —— 原版 `ChooseCardMenu` 全树也只有两个调用点）。⚠️ 这里原来写「没有（引擎没有『选一张牌』的效果）」，**是过期记录** |
| **单位语音条** | `Unit Chat/PlayerChatDisplay` 648.8×236.5（`40k_voicelines_radio` 766×280）+ 敌侧同尺寸 | ① `.../FrontCanvas` | 没有（原版有语音，我们没接音频）。**下一步**（第 4 行剩的第 1 件）。✅ **音频放 `Resources/` 已拍板**（2026-09-16，见 `项目任务.md` 待办表第 4 行） |
| **回放条** | `ReplayButtons` 4 枚 79.8×48.6（`40K_replay_bt_*`） | ① `.../BackCanvas` | 没有（单机没回放）。**动手前先复核坐标悬案**（坑 35 vs 坑 38） |
| **等待提示 / 通用弹窗** | `WaitText` 1344×79.4 + `Dark Shade` 3963.5×3366 + `40k_popup` 1323×90 | ① `.../BackCanvas` | ✅ **2026-09-17 做了**（`Battle/WaitBanner.cs`，对手回合显示；位置/尺寸/α 都有断言）。⚠️ **触发时机与文案是我们挑的**（原版查不到），见该文件头 |
...
## 三·〇、⏭ 第 4 行「界面剩余件」的**执行计划**（2026-09-17 立，动手照它走）

> 立项依据：`项目任务.md` 顶部表**第 4 行**。**顺序不冲突** —— 那张表把第 4 行明确列进
> 「**彼此不依赖、可自由穿插的**」（第 4 / 5 / 8 / 10 / 11 / 12 行「各自只碰一两个文件」），
> 推荐顺序也是 `1 → 2 →（**4、5、11 穿插**）→ 3+6 → 7 → 8、10`。
> 而第 1 行只剩「`-wfhuman` 偶发卡死（测试台问题）」、第 2 行只剩「不搜索 / 不留牌保命」⇒ **这里正是插入点**。
> ⚠️ **软性风险**：那条卡死没定位，做完要进真包验证时可能撞上（不阻塞）。

**四件的规格与现状**（规格见上面 §三 :72-74，都是原版实测值）

| 序 | 件 | 原版规格 | 现状（**2026-09-17 逐条核过**） | 体量 |
|---|---|---|---|---|
| ~~1~~ | ~~点开牌堆看张数~~ | `DeckManager__ToggleCardbackPreview` 只有一句 `cardbackContainer.SetActive(state)` | ✅ **2026-09-17 结论：别做，没东西可做**（三条实据见下）。唯一真缺的是原版那层 `Cardback Shadow SDF`（要新导出 4 个阵营，独立小事） | — |
| ~~2~~ | ~~等待提示 / 通用弹窗~~ | `WaitText` 1344×79.4 + `Dark Shade` 3963.5×3366（纯色 α 0.6118）+ `40k_popup` 1323×90 | ✅ **2026-09-17 做完了** → `Battle/WaitBanner.cs` + 6 条断言（`BattleScene.Run` 456/456）。⚠️ **底板没用原版 `40k_popup` 九宫格**（我们没有九宫格，`SettingsPanel` 撞过同一堵墙）⇒ 自建实底，标成「我们的做法」 | — |
| 3 | **回放条** | `ReplayButtons` @ `BackCanvas` anchor(0.5,1) pos(−403,−66) **293.60×57.41**；`Holder` 下 **4 枚各 79.80×48.57**（图 95px × **0.84**）；⚠️ **`Play`/`Pause` 同座标互斥切换**（同屏只可见 3 枚） | 没有。⚠️ **两处冲突动手前必须自己定**（见下） | 中 |
| 4 | **单位语音条** | `PlayerChatDisplay` anchor(0,0) pos(12.61,200) **648.77×236.5** → x[12.6,661.4] y[643.5,880.0]；`EnemyChatDisplay` anchor(0,1) pos(12.61,−410) → **y[173.5,410.0]**。子件：`Background` 图 `40k_voicelines_radio`(766×280) · `wave` 图 `40k_voicelines_radio_wave equalizer` · `ChatText` **fontSize 33 白** | 没有，**且工程里一条音频都没有**。但**图已进工程**（`40k_voicelines_radio` + 4 张 `40k_UnitChat_Background_*`）。⚠️ **`…wave equalizer` 工程里没有**，波形那层要新切 | 大 |

**🔴 第 1 件为什么「别做」（2026-09-17 三条实据，别再翻）**：
1. **「看张数」早做完了** —— `BattleDriver.cs:2992-3002` 的张数底板 + 文字有断言守着。
2. **牌背常显是对的** —— `UI_Deck_Background` 那张图是「卡形深色框」，牌背 217×314 **比框还高**（插在框里高出框外）；
   原版 `DisplayDeckSize` 里就有「一并把牌背打开」的分支。之前读到的「`Cardback Container` 默认关」
   **是预制体静态值**（那几份 dump 的路径根都是 `BattlePrefab/BattlePrefab/…`，连 `PlayerDeck` 自己都写着 `activeSelf=False`）
   ⇒ **不是运行时结论**，别拿它改代码。
3. **原版的点击入口查不到** —— `ToggleCardbackPreview` 反编译 **0 调用点**、场景 JSON **0 命中**（可能是死代码）。
   **任何手势都是我们编的**，那只会让游戏离原版更远。
⇒ 剩下唯一的真缺口是 **`Cardback Shadow SDF`（原版 292×381）没进工程**（`Cardback_*_SDF` sprite 原版有 699 个，
   工程里 `back_*.png` 只有 4 个且没有 SDF 版）——**独立小事，要做单开一条**。

**⏭ 第 4 行剩下的两件，按这个顺序**（第 1、2 件已了结）：
**③ 回放条**（先复核坐标悬案）→ **④ 单位语音条**（最大的一件：导音频 + 建映射 + 做 `PlayerChatDisplay` 面板）。

**🔴 第 3 件动手前必须自己定的两处冲突**：
1. **坐标**：`解包资源使用地图.md` **坑 35**（屏外、观战才滑入）vs **坑 38**（`chain_rect` 证明**屏内顶部 x[410.2,703.8] y[37.3,94.7]**）。
   坑 38 在后且写明教训「任何『屏外/错位』结论**必须 `chain_rect` 复核**」⇒ **以坑 38 为准，但自己复核一遍**。
2. **`Play`/`Pause` 的 activeSelf**：dump（① `runtime_ui_dump_drive_0912.tsv:329-330`）说 `Play=False/Pause=True`；
   权威表（③ `战斗界面JSON权威表_0827.md:212-214`）说 `Play=active/Pause=inactive` —— **正好相反**。
   ⚠️ 这也**推翻了 §一 :15 那句「①②逐字段一致」**（至少这一项不成立）。

**⚠️ 四件共同的「查不到」**（都**不在**文档里，做之前要知道自己是在**挑**还是在**复刻**）：
- **`Unit Chat` 的播放触发点** —— 单位语音在什么事件播、谁调、`ChatText` 与音频怎么同步，**文档与反编译都没有**。
  `语音索引.md` §四 只从**文件名**推事件集（`greet/attack/death/concede/gen1-7`），且明写 **`wp` 语义未证实**。
- **`ReplayButtons` 的触发时机** —— 原版什么模式才出现（观战？回放？）**查不到**。「单机没回放」是**我们的选择**，不是原版规格。
- **`ToggleCardbackPreview` 的点击入口** —— **反编译 0 调用点、场景 JSON 0 命中**。要接就得自己定手势 ⇒ **照文档做会变成「我们挑的」**。
- **`DisplayDeckSize` 的文案格式** —— 两个字面量 `StringLiteral_13658`/`_23310` **未解码**；我们现在的 `DECK N`（`:3482`）是**自拟**。

**✅ 现成能用的两样**（别再挖）：
- **单位语音的权威映射**：`D:/4/Unity/数据/索引/card_index.json` 的 `cards[].voice`（**1193 张卡 / 1778 条引用**）。
- **音频源**：取 `D:/2/新解包资源/assets_full/bundle_*cardassets_assets_all/AudioClip/` 那一份（`D:/2/解包整理/01_卡牌/*/AudioClip/` 是**同内容第二份**）。
  ⚠️ **别只 copy `*.ogg`** —— 还有 **`VO_*.wav` 115 条**（DarkAngels 31 + Ultramarines 84），
  **漏了就没有 `Guilliman` / `Valius Paxor` 两个督军**。合计每份 1813 条 / **42.5 MB**。

**执行顺序 = 由小到大**（先拿到绿再做大的）：1 → 2 → 3 → 4。

**各件的坑 / 前置**

- **③ 回放条要先解一个坐标悬案**：`索引与盘点/解包资源使用地图.md` 里
  **坑 35**（`LeftArea` 相对 [-550,1117] 屏外、观战才滑入）与
  **坑 38**（`chain_rect` 权威证明**在屏内顶部 x[410.2,703.8] y[37.3,94.7]**）**互相矛盾**。
  坑 38 在后、且写明**教训**：「任何『屏外/错位』结论**必须 `chain_rect` 复核**后再动代码」⇒ **以坑 38 为准，但动手前自己复核一遍**。
- **④ 语音条真正缺的是「把 ogg 导进工程 + 接 UI」**（`可并行任务清单.md` 第 ⑧ 条明写这句，且注明**这是 Unity 活、不能派**）。
  已知：`VO_*.ogg` **1698 条**（另有 `VO_*.wav` 115 条、死灵族 44 条不带 `VO_` 前缀）≈ **42 MB**，
  按动作分 `greet/attack/death/concede/gen1-7`；**索引现成**（`资料/普查产出_0913/语音索引.md`）。
  ⚠️ **工程里不止缺 VO —— 一条音频都没有**（617 条音效 / 43 条督军语音 / 2 首音乐也都只在 `素材/Warpforge原版/音频/`，没导进 `Assets/`）。
  ⚠️ **待拍板**：音频放 `Resources/`（进包、构建再涨 ~90 MB）还是 `StreamingAssets/`（不进包、运行时读）。
- **② 与 §六「我们加的」不冲突**：`Dark Shade` / 通用弹窗我们本来没有，照原版做即可。

**能派的 / 不能派的**（铁律 1·b）
- ✅ **能派**：「`40K_replay_bt_*` / `40k_voicelines_radio` / `40k_popup` 这几张图**在本地哪、切片矩形多少**」
  （查 `索引与盘点/解包资源使用地图.md` → `ui_extract` → 全盘搜，三步走完）· 回放条坐标悬案的独立复核。
- ❌ **不能派**：改 `BattleDriver` / `BattleScene` 代码、跑自检 —— 同工程同时只能一个 Unity 实例，且都在同几个文件里。
...
| ~~**选卡菜单**~~ **等待提示 / 回放条** 那几套 | — | ⏳ **还没做**（结构与图多半在本地，见 §三点七）。⚠️ 换牌（Mulligan）不在这里 —— 它已经做完了。**选卡菜单 2026-09-14 已做**（三族共用一个面板，见 `资料/选牌Choose_数据与设计.md`） |
