...
CARDS = [
    # ── 2026-09-16 下半场「±N 属性与卡面图标不符」批（31 张改 / 2 张是我们对的）──
    ("AM22", "第 2 个属性该是**紫枪=远程**（原 token `[Attack]`）"),
    ("DA4",  "第 2 个 `+1` 该是**紫枪=远程**（原 `[Armor]`）"),
    ("EC8",  "两个都该是**拳 / 枪**（原 `[Health]` / `[Attack]`）"),
    ("GOF87", "第 2 个该是**枪**（原 `[health]`）—— ⚠️ 图标表的锚点会把它配成盾"),
    ("SOR18", "该是**拳=近战**（原 `[Armor]`）"),
    ("SW19", "第 2 个 `+3` 该是**枪**（原 `Armor`）"),
    # ── 同轮：A8 / A9 / 英文 desc 三条尾巴 ──
    ("ASH_Autarch", "句首该有灵魂石标记 `①`（原来整段漏印）"),
    ("ASH49", "句首**不该**有 `②`（原版没印，那是右上角蓝色费用圈）"),
    ("TL33", "关键词该带数字：`护甲 1。爆裂 2。`（原来印成裸词）"),
    ("SOR42", "该有 `8 ☀` 前缀（英文 `desc` 原来连前缀都没有）"),
    ("UM_Primaris_Chaplain", "句首该有「典籍：」（**只改了显示层**，英文 desc 故意保持裸写）"),
    ("SOR48", "该是 `〔盾〕Shield`，不是 `+1 Attack`"),
    # ── 2026-09-16 下半场：卡面版面的四件尾巴 + 三条从没译过的中文 ──
    ("EC41", "两位数 `10` 该缩号进紫圈（原来挤出圆外）"),
    ("AM5", "立绘该是**持动力锤的老妇人**（原来配成了扛 CADIA 旗的替身画）"),
    ("UM83", "中文该印出来（原来 `descZh` 是空 ⇒ 整段英文）"),
    ("BL2", "同上；顺带看 `or` 那句是不是「二选一」的措辞"),
    ("TAU23", "同上；`远射。猛击：…标记光 2`"),
]
